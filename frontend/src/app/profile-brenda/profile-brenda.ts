...
export class ProfileBrenda implements OnInit {
  ...
  async ngOnInit() {
    const data = await this.backendService.getOne('brenda');
    this.profile.set(data);
    console.log('Profile-brenda:', this.profile());
    this.selectedHeader = this.profile()[0].headerImage || 'brendaTitle1.png';
  ...
  async deleteOne(index: number) {
    await this.backendService.deleteOne('brenda', index);
    const data = await this.backendService.getOne('brenda');
    this.profile.set(data);
  }
  ...
  async onUploadFiles() {
    ...
    await this.backendService.update('brenda', this.profile()[0]);

    const updated = await this.backendService.getOne('brenda');
  ...
  selectedHeader = 'brendaTitle1.png';
