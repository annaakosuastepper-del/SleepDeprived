...
export class ProfileHari implements OnInit {
  ...
  // TODO: pick the real prev/next order across all four profiles (anna/hari/nina/brenda).
  // Guessed cycle for now: anna -> hari -> nina -> brenda -> anna
  prevProfile() {
    this.router.navigate(['/profile/brenda']);
  }

  nextProfile() {
    this.router.navigate(['/profile/nina']);
  }
